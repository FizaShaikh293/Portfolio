...
export default function ArchitectureCover({ stages, accent = 'text-primary' }: Props) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-border bg-muted/40 p-4 sm:p-5">
      ...
      <div className="relative flex flex-col sm:flex-row items-stretch gap-2 sm:gap-0">
        {stages.map((s, i) => (
          <div key={s.label} className="flex-1 flex flex-col sm:flex-row items-center gap-2 sm:gap-0 min-w-0">
            <div className="w-full rounded-lg border border-border bg-background/40 backdrop-blur-sm px-3 py-2.5 transition-colors duration-300 hover:border-primary/30">
              <div className={`text-[9px] uppercase tracking-[0.18em] ${accent} mb-1.5`}>{s.label}</div>
              <div className="flex flex-wrap gap-1">
                {s.items.map((it) => (
                  <span
                    key={it}
                    className="text-[10px] font-mono text-muted-foreground/90 rounded border border-border px-1.5 py-0.5"
                  >
                    {it}
                  </span>
                ))}
            ...
            {i < stages.length - 1 && (
              <div className="shrink-0 sm:w-6 sm:h-px h-5 w-px relative overflow-hidden bg-muted/40 sm:mx-1">
